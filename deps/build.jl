...
if Sys.isunix() # is_unix()
    ...
    src02 = joinpath(srcDir, "Dipole1D.f90")
    src03 = joinpath(srcDir, "callDipole1D.f90")
    dirfile = joinpath(libDir, "Dipole1D.dll")

    printstyled("=== Fortran compilier Info ===\n", color=:blue)
    run(`C:\\msys64\\mingw64\\bin\\gfortran.exe --version`)
    run(`C:\\msys64\\mingw64\\bin\\gfortran.exe -O2 -fPIC -shared $(src01) $(src02) $(src03) -o $(dirfile)`)
